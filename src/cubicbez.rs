...
fn one_coord_extrema(result: &mut ArrayVec<f64, MAX_EXTREMA>, d0: f64, d1: f64, d2: f64) {
    let a = d0 - 2.0 * d1 + d2;
    let b = 2.0 * (d1 - d0);
    let c = d0;
...
impl ParamCurveExtrema for CubicBez {
    fn extrema_x(&self) -> ArrayVec<f64, MAX_EXTREMA> {
        ...
        one_coord_extrema(&mut result, d0.x, d1.x, d2.x);
    ...
    fn extrema_y(&self) -> ArrayVec<f64, MAX_EXTREMA> {
        ...
        let d1 = self.p2 - self.p1;
        let d2 = self.p3 - self.p2;
        one_coord_extrema(&mut result, d0.y, d1.y, d2.y);
        result.sort_by(|a, b| a.partial_cmp(b).unwrap());
        result
    }
}
