...
impl PathSectionInfo<'_> {
    pub fn new(index: usize, path_index: usize, section_path: &BezPath) -> PathSectionInfo {
        PathSectionInfo {
            index,
            path_index,
            section_path,
...
fn populate_elements_list<'a>(
    ...
    let mut test = sections
        .iter()
        .enumerate()
        .map(|(index, path)| {
            // Create an info struct
            PathSectionInfo::new(elements.len() + index, path_index, &path)
        })
        .collect::<Vec<_>>();
...
fn flag_path_sections_to_keep(
    ...
    process_winding_results: &dyn Fn(
        Vec<(f64, i32, usize, usize, bool)>,
        &mut Vec<PathSectionInfo>,
    ),
    accuracy: f64,
) -> Vec<PathEl> {
    while let Some(info1) = elements.iter().find(|info| !info.processed) {
        ...
        let ray_is_horizontal =
            info1.section_bounds.width().abs() <= info1.section_bounds.height().abs();
        let ray_position = if ray_is_horizontal {
            (info1.section_bounds.min_y() + info1.section_bounds.max_y()) / 2.
        } else {
            (info1.section_bounds.min_x() + info1.section_bounds.max_x()) / 2.
        };
        let ray_point = if ray_is_horizontal {
            Point::new(info1.section_bounds.max_x() + 1., ray_position)
        } else {
            Point::new(ray_position, info1.section_bounds.max_y() + 1.)
        };
        ...
        let mut winding_results = elements
            ...
            .by_ref()
            .map(|info2| {
                let winding_array = ray_fn(&*info2.section_path, ray_point)
                    .into_iter()
                    .map(|(pt, winding)| {
                        (
                            pt,
                            winding,
                            info2.index.clone(),
                            info2.path_index.clone(),
                            info2.processed.clone(),
                        )
                    })
                    .collect::<Vec<_>>();
                winding_array
        ...
        process_winding_results(winding_results, elements);
    }
    ...
    let mut elements = elements
        ...
        .filter_map(|info| {
            if info.keep {
                Some((info.section_path, false, false))
            } else {
                None
            }
        })
    ...
    for index1 in 0..elements.len() {
            ...
            .get_seg_end(elements[index1].0.elements().len() - 1)
        {
            ...
            let mut candidates = elements
                ...
                .map(|(index2, (path2, _, _))| {
                    (
                        index2,
                        path2.get_seg_end(0).unwrap(),
                        path2.get_seg_end(path2.elements().len() - 1).unwrap(),
                    )
                })
                .filter(|&(_, start, end)| {
                    (start - path_end1).hypot2() <= accuracy2
                        || (end - path_end1).hypot2() <= accuracy2
                })
                .collect::<Vec<_>>();
            candidates.sort_by(|&(_, start1, end1), &(_, start2, end2)| {
                let d1 = (start1 - path_end1)
                    .hypot2()
                    .min((end1 - path_end1).hypot2());
                let d2 = (start2 - path_end1)
                    .hypot2()
                    .min((end2 - path_end1).hypot2());
            ...
            if let Some(&(connected_index, start, end)) = candidates.get(0) {
                ...
                elements[connected_index].2 =
                    (start - path_end1).hypot2() > (end - path_end1).hypot2();
    ...
    elements
        ...
        .map(|(path, connect_to_previous, reverse)| {
            ...
            BezPath::from_vec(
                p_correct.elements()[if connect_to_previous { 1 } else { 0 }..].to_vec(),
            )
        })
        .flatten()
        .collect::<Vec<_>>()
...
pub fn convert_path_to_even_odd(path: &BezPath, accuracy: f64) -> BezPath {
    ...
    let elements = flag_path_sections_to_keep(&mut elements, &process_winding_results, accuracy);
        ...
        |winding_results: Vec<(f64, i32, usize, usize, bool)>,
         elements: &mut Vec<PathSectionInfo>| {
            winding_results
                ...
                .scan(
                    (0, 0),
                    |winding_total, (_, w, index, path_index, processed2)| {
                        if path_index == 0 {
                            winding_total.0 = winding_total.0 + w
                        } else {
                            winding_total.1 = winding_total.1 + w
                        };
                ...
                .for_each(|(w_current, index, path_index, _)| {
                    ...
                    info.keep = (is_path_a
                        && closed_flags.contains(ClosedPathOperationsFlags::KEEP_A_OUTSIDE_B)
                        && !inside_path_b)
                        || (is_path_a
                            && closed_flags.contains(ClosedPathOperationsFlags::KEEP_A_INSIDE_B)
                            && inside_path_b)
                        || (!is_path_a
                            && closed_flags.contains(ClosedPathOperationsFlags::KEEP_B_INSIDE_A)
                            && inside_path_a)
                        || (!is_path_a
                            && closed_flags.contains(ClosedPathOperationsFlags::KEEP_B_OUTSIDE_A)
                            && !inside_path_a);
